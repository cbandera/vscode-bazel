...
export abstract class BaseExtensionFeature implements vscode.Disposable {
  ...
  private onConfigurationChanged(config: vscode.WorkspaceConfiguration): void {
    ...
    if (shouldBeEnabled && !this.isEnabled) {
      this.logger.info(`Enabling ${this.constructor.name}`);
      if (!this.enable(this.context)) {
        this.logger.error(`Failed to enable ${this.constructor.name}`);
        void vscode.window
          .showErrorMessage(
            `Bazel: Failed to enable ${this.featureName} feature.`,
            "Show Output",
          )
          .then((selection) => {
      ...
      this.logger.info(`Disabling ${this.constructor.name}`);
      if (!this.disable()) {
        this.logger.error(`Failed to disable ${this.constructor.name}`);
        return;
      }
